...
Usage:
    1. Start the MCP server:  cd pkg/agents/clickhouse && python server.py --transport sse
    2. Run chatbot:           source .venv/bin/activate && python scripts/clickhouse/chatbot.py
"""
...
SCHEMA_CONTEXT = """
Database: ecommerce  (ClickHouse, MergeTree engine)
...
def nl_to_sql(question):
    ...
    prompt = (
        ...
        "- Use ClickHouse-compatible syntax (count(), toString(), etc.).\n"
...
async def mcp_execute_query(session, sql):
    ...
    result = await session.call_tool("ch_execute_query", {"sql": sql, "limit": 200})
    parsed = _parse_mcp_result(result)

    # Response shape: {"query": ..., "results_per_instance": {"inst_name": {"columns": [...], "rows": [...], ...}}}
    ...
    try:
        async with mcp_client as session:
            ...
            try:
                ...
                print(DIM + "    cd pkg/agents/clickhouse && python server.py --transport sse" + RESET + "\n")
                sys.exit(1)
...
""" + RESET)
        ...
        print(BOLD + "    cd pkg/agents/clickhouse && python server.py --transport sse" + RESET + "\n")
        sys.exit(1)
